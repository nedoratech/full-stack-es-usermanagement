...
{
    internal static CloudEvent<object> ToCloudEvent<T>(this T @event) where T : class
    {
        if (@event == null)
    ...
    private static Guid GetAggregateId<T>(T @event) where T : class
    {
    ...
    private static DateTime GetOccurredAt<T>(T @event) where T : class
    {
